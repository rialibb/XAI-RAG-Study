...
def retrieve_with_contriever(question, chunks, k, approach="cosine"):
    ...
    doc_inputs = contriever_tokenizer(
    ...
    if approach == "cosine":
        ...
        x = np.concatenate(
            [question_emb.cpu().detach().numpy(), doc_embs.cpu().detach().numpy()]
        )
...
def extract_rationale(generator, query, contexts, answer):
    """
    Extracts the most relevant context spans that justify the generated answer using an LLM prompt.

    Args:
        generator (pipeline): The LLM generator pipeline.
    ...
    rationale_output = (
        generated_text.split("Important supporting context:")[-1]
        .split("</think>")[-1]
        .strip()
    )
...
def extract_metrics(
    generator, prompt_function, output_modificator, query, contexts, answer
):
    """
    Extracts the most relevant context spans that justify the generated answer using an LLM prompt.
    ...
    Args:
        generator (pipeline): The LLM generator pipeline.
        prompt_function (function): Function to create the prompt for the generator.
        output_modificator (function): Function to modify the output of the generator.
        query (str): The original user question.
        contexts (list of str or tuples): List of context chunks used for answering the query.
        answer (str): The generated answer to be justified.

    Returns:
        str: Formatted output as detailed in prompt_function.
    """
    ...
    context_text = "\n".join(contexts)
    # The prompt function structure should always be the same i.e. the inputs should be (query, contexts, answer)
    # and the output should be a string that is the prompt to be passed to the generator
    prompt = prompt_function(query=query, contexts=context_text, answer=answer)
    result = generator(prompt, max_new_tokens=512)
    generated_text = result[0]["generated_text"]
    return output_modificator(generated_text)
